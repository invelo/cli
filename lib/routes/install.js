...
var getAllGenerators = (function () {
  ...
  return function (cb) {
    ...
    npmKeyword('invelo-generator', function (err, packages) {
      if (err) {
        cb(err);
        return;
      }
...
function searchMatchingGenerators(app, term, cb) {
  got('http://invelo.io/blacklist.json', function (err, data) {
...
function isInveloPackage(pkg) {
  if (pkg.author && pkg.author.name) {
    return pkg.author.name.indexOf('Invelo') !== -1;
  }
...
function fetchGeneratorInfo(generator, cb) {
  packageJson(generator.name, function (err, pkg) {
    ...
    var official = isInveloPackage(pkg);
    var mustache = official ? chalk.green(' ' + figures.mustache) : '';

    cb(null, {
      name: generator.name.replace(/^@invelo\//, '') + mustache,
